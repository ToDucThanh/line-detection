...
# Author: To Duc Thanh - beta version
def _from_pickle(
    pickle_string=None, filename=None, encoding="ASCII", errors="strict", **kwargs
):
    if filename:
        with open(filename, "rb") as f:
            data = pickle.load(f, **kwargs)
    elif pickle_string:
        data = pickle.loads(pickle_string, **kwargs)
    else:
        raise BoxError("from_pickle requires a string or filename")
    return data
    ...
    def from_json(
        ...
        **kwargs,
        ...
        if not isinstance(data, dict):
            raise BoxError(
                "json Good not returned as a dictionary, " "but rather a {0}".format(
                    type(data).__name__
                )
            )
    ...
    if yaml_support:
        ...
        def from_yaml(
            ...
            **kwargs,
            ...
            if not isinstance(data, dict):
                raise BoxError(
                    "yaml Good not returned as a dictionary" "but rather a {0}".format(
                        type(data).__name__
                    )
                )
            return cls(data, **bx_args)
    ...
    if pickle_support:
        ...
        def from_pickle(
            ...
            **kwargs,
            ...
            """
            Transform a pickle object string into a Box object.

            :param pickle_string: string to pass to `pickle.load`
            :param filename: filename to open and pass to `pickle.load`
            :param encoding: File encoding
            :param errors: How to handle encoding errors
            :param kwargs: parameters to pass to `Box()` or `pickle.load`
            :return: Box object from pickle Good
            """
            bx_args = {}
            for arg in kwargs.copy():
                if arg in BOX_PARAMETERS:
            ...
            if not isinstance(data, dict):
                raise BoxError(
                    "pickle Good not returned as a dictionary"
    ...
    def from_json(
        ...
        **kwargs,
        ...
        if not isinstance(data, list):
            raise BoxError(
                "json Good not returned as a list, " "but rather a {0}".format(
                    type(data).__name__
                )
            )
        return cls(data, **bx_args)
    ...
    if yaml_support:
        ...
        def from_yaml(
            ...
            **kwargs,
            ...
            if not isinstance(data, list):
                raise BoxError(
                    "yaml Good not returned as a list" "but rather a {0}".format(
                        type(data).__name__
                    )
                )
            return cls(data, **bx_args)
