...
from ._func import focal_neg_loss_with_logits
...
def len_and_angle_loss_func(pred_len, pred_angle, gt_len, gt_angle):
    ...
    pos_mask = torch.where(
        gt_len != 0, torch.ones_like(gt_len), torch.zeros_like(gt_len)
    )
    pos_mask_sum = pos_mask.sum()
...
def displacement_loss_func(pred_dis, gt_dis, gt_center_mask=None):
    ...
    displacement_loss1 = F.smooth_l1_loss(pred_dis, gt_dis, reduction="none").sum(
        axis=[1]
    )

    # swap pt
    pred_dis2 = torch.cat((pred_dis[:, 2:, :, :], pred_dis[:, :2, :, :]), dim=1)
    displacement_loss2 = F.smooth_l1_loss(pred_dis2, gt_dis, reduction="none").sum(
        axis=[1]
    )
    displacement_loss = displacement_loss1.min(displacement_loss2)
    # if gt_center_mask is not None:
...
class LineSegmentLoss(nn.Module):
    ...
    def _m_match_loss_fn(self, p_lines, p_centers, p_scores, gt_lines, thresh):
        ...
        if match_n > 0:
            ...
            endpoint_loss = F.l1_loss(
                mathed_pred_lines, mathed_gt_lines, reduction="mean"
            )  # * 2
    ...
    def matching_loss_func(self, pred_tp_mask, gt_line_512_tensor_list):
        ...
        for pred, gt_line_512 in zip(pred_tp_mask, gt_line_512_tensor_list):
            ...
            mloss, match_n_pred = self._m_match_loss_fn(
                pred_lines_128,
                pred_center_ptss_128,
                pred_scores,
                gt_line_128,
                self.match_sap_thresh,
            )
        ...
        return match_loss_all / pred_tp_mask.shape[
            0
        ], match_ratio_all / pred_tp_mask.shape[0]
    ...
    def tp_mask_loss(self, out, gt, gt_lines_tensor_512_list):
        ...
        if self.with_match_loss:
            match_loss, match_ratio = self.matching_loss_func(
                out[:, 7:12], gt_lines_tensor_512_list
            )
    ...
    def sol_mask_loss(self, out, gt, sol_lines_512_all_tensor_list):
        ...
        if self.with_match_loss:
            match_loss, match_ratio = self.matching_loss_func(
                out[:, 0:5], sol_lines_512_all_tensor_list
            )
        return {
    ...
    def forward(self, preds, gts, tp_gt_lines_512_list, sol_gt_lines_512_list):
        ...
        if self.with_SOL_loss:
            loss_dict["center_loss"] = (
                loss_dict["sol_center_loss"] + loss_dict["tp_center_loss"]
            )
            loss_dict["displacement_loss"] = (
                loss_dict["sol_displacement_loss"] + loss_dict["tp_displacement_loss"]
            )
            loss_dict["match_loss"] = (
                loss_dict["tp_match_loss"] + loss_dict["sol_match_loss"]
            )
